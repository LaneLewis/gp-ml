...
from gaussian_process import sde_kernel_matrices
from torch import optim

class GPML_VAE():
    def __init__(self,device:str,latent_dims:int,observation_dims:int,timesteps:int,
                 encoder_function:function,decoder_function:function,
    ...
    def fit(self,X_train:torch.Tensor,encoder_optimizer:object,decoder_optimizer:object,epochs=100,
            lr=0.001,optimize_taus=True,optimize_R=True,optimize_encoder=True,optimize_decoder=True,
            batch_size=1):
        '''
        ...
        optimize_encoder - bool: indicates whether or not to run the optimizer on the encoder
        optimize_decoder - bool: indicates whether or not to run the optimizer on the decoder
        batch_size - int: indicates the batch size to split the data into for each gradient step
        '''
        #NOTE make dummy class to act as an empty optimizer
        param_optimize_list = []
        if optimize_taus:
        ...
        #NOTE this might break if adam gets a zero len list, if so handle this
