...
class GPML_VAE(nn.Module,GPMLDataStruct):
    ...
    def fit(self,X_train:torch.Tensor,X_validation:torch.Tensor,lr=0.001,epochs=100,batch_size=1,approx_elbo_loss_samples=100,
            log_save_name="latest",hyper_scheduler=None,save_cycle=100,grad_clip=0.01,tau_lr=None):
        '''
        ...
        self.kl_loss = []
        #if tau_lr is None:
        #    optimizer = torch.optim.Adam([{"params":self.log_taus},{"params":self.parameters()}],lr=lr)
        #else:
        #    optimizer = torch.optim.Adam([{"params":self.log_taus,"lr":tau_lr},{"params":self.parameters()}],lr=lr)
        ...
        for epoch in tqdm(range(epochs),desc=f"VAE Fit",colour="cyan"):
            ...
            if epoch%save_cycle == 0:
                ...
                plot_R_diag(self.R_diag_trajectory,log_save_name)
...
def plot_R_diag(epochs_R_diag,filename,average=99):
    epochs = range(len(epochs_R_diag))
    plt.title("GPML MC: Loss Over Epochs")
    plt.plot(epochs,epochs_R_diag,label=f"{epochs_R_diag[-1]}")
    if len(epochs_R_diag) > average:
        avgs = np.mean(np.array(epochs_R_diag)[-average:,:],axis=0)
    else:
        avgs = np.mean(np.array(epochs_R_diag),axis=0)
    for v in avgs:
        plt.axhline(v,label=f"mean taus: {v}",color="red")
    plt.ylabel("R")
