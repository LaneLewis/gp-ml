...
def two_latent_three_pop_dataset(time_start=0.0,time_end=10.0,time_divisions=10,
            R_diag = [5.0,5.0,5.0],taus=[4.0,5.0],signal_sds = [0.98,0.98],
    ...
    Z,X = true_model.sample_model(samples)
...
def two_latent_three_pop_MC(train_dataset,test_dataset,layers=[(20,nn.GELU())],epochs=100,batch_size=10,initial_taus=[5.0,9.0],
    ...
    decoding_optimizer =  optim.Adam(decoding_model.parameters(),lr = lr)
    gpml = GPML_LLF(device,latent_dims,observed_dims,train_dataset["times"],decoding_model,initial_taus=initial_taus,signal_sds=signal_sds,noise_sds=noise_sds,initial_R_diag=initial_R_diag)
    #add validation set
    gpml.fit_generative_model(train_dataset["X"],test_dataset["X"],decoding_optimizer,optimize_taus=True,optimize_R=True,batch_size=batch_size,epochs=epochs,approx_log_likelihood_loss_samples=samples,
                                  tau_lr=0.005,R_diag_lr=0.005,print_batch_values=False)
...
def two_latent_three_pop_VAE(train_dataset,test_dataset,encoding_model_layers=[(50,nn.GELU()),(50,nn.GELU())],decoding_model_layers=[(20,nn.GELU())],epochs=100,batch_size=10,initial_taus=[5.0,9.0],
    ...
    decoding_model = FeedforwardNNDecoder(decoding_model_layers,latent_dims,observed_dims)
    decoding_optimizer = optim.Adam(decoding_model.parameters(),lr=0.001,amsgrad=True)
    #add validation set
    ...
    gpml.fit(train_dataset["X"],test_dataset["X"],encoding_optimizer,decoding_optimizer,optimize_taus=True,optimize_R=True,batch_size=batch_size,epochs=epochs,approx_elbo_loss_samples=samples)
...
train_dataset = two_latent_three_pop_dataset(R_diag=[1.0,0.5,0.8],time_divisions=12,samples=20)
with open(f"./midway_models/train_dataset.pkl","wb") as f:
    pkl.dump(train_dataset,f)
test_dataset = two_latent_three_pop_dataset(R_diag=[1.0,0.5,0.8],time_divisions=12,samples=10)
...
#two_latent_three_pop_MC(train_dataset,test_dataset,epochs=1000,batch_size=10,samples=5000,run_save_name="second_latest",layers=[(20,nn.LeakyReLU()),(20,nn.LeakyReLU())])
#graph_MC("second_latest")
    
two_latent_three_pop_VAE(train_dataset,test_dataset,epochs=200000,samples = 5,encoding_model_layers=[(200,nn.LeakyReLU()),(200,nn.LeakyReLU())],decoding_model_layers=[(20,nn.LeakyReLU()),(20,nn.LeakyReLU())])
