...
class Test_LL_Fit(unittest.TestCase):
    ...
    def test_LL_fit_nn_dataset(self):
        ...
        #initial parameters
        model_latent_dims = 2
        model_taus = [15.0,15.0]
        signal_sds = model_latent_dims*[0.99]
        noise_sds = model_latent_dims*[0.01]
        model_R_diag = [15.0,15.0,15.0]
        #Fitting parameters
        generative_fit_epochs = 1000
        decoding_model = FeedforwardNNDecoder([(10,nn.Sigmoid()),(10,nn.Sigmoid())],model_latent_dims,observed_dims,device=device)
        decoding_optimizer = optim.Adam(decoding_model.parameters(),lr=0.005)
        ...
        gpml.fit_generative_model(X,decoding_optimizer,optimize_taus=True,optimize_R=True,batch_size=20,epochs=generative_fit_epochs,approx_log_likelihood_loss_samples=30000,
