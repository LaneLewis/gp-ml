...
class GPML_VAE_Alt():
    def __init__(self,device:str,latent_dims:int,observation_dims:int,times:torch.Tensor,
        ...
        decoder_model - .parameters() which returns the an iterator on the model parameters
                        .forward(Z) -> manifold_mean
                            ...
                            Z has shape [batch_size,samples,timesteps,latent_dims]
        '''
    ...
    def fit(self,Z,X_train:torch.Tensor,encoder_optimizer:object=None,decoder_optimizer:object=None,epochs=100,
            tau_lr=0.001,R_diag_lr =0.001, optimize_taus=True,optimize_R=True,
            batch_size=1,approx_elbo_loss_samples=100,loss_hyperparameter=1.0,encoder_pass_taus=True,decoder_pass_taus=True):
        '''
        ...
        for epoch in range(epochs):
            ...
            for (batch_X,) in tqdm(batched_X_train,desc=f"epoch: {epoch}",colour="cyan"):
                ...
                indiv_losses,indiv_kls,indiv_lls = approx_elbo_loss(self.taus,batch_X,self.encoder_model.forward,self.decoder_model.forward,self.R,self.kernel_matrices,
                                                                    samples=approx_elbo_loss_samples,loss_hyper=loss_hyperparameter,taus_to_encoder=encoder_pass_taus,taus_to_decoder=decoder_pass_taus)
            ...
            print(f"X Reconstruction: {self.reconstruction_loss(X_train,encoder_pass_taus=encoder_pass_taus,decoder_pass_taus=decoder_pass_taus)}")
            print(f"Z Reconstruction: {torch.linalg.norm(Z - self.predict_Z(X_train,encoder_pass_taus=encoder_pass_taus)).numpy()}")
    ...
    @torch.no_grad
    def predict_Z(self,X,encoder_pass_taus=False):
        #maybe its not predicting the right Z, reshape issue somewhere
        means,_ = self.encoder_model.forward(X,self.taus)
        return means
    
    @torch.no_grad
    def predict_Z_std(self,X,encoder_pass_taus=False):
        _,stds = self.encoder_model.forward(X,self.taus)
        return stds
    
    @torch.no_grad
    def reconstruction_loss(self,X,encoder_pass_taus=False,decoder_pass_taus=False):
        if encoder_pass_taus:
            means, _ = self.encoder_model.forward(X,self.taus)
        else:
            means, _ = self.encoder_model.forward(X)
        mean_sample = means.expand(1,means.shape[0],means.shape[1],means.shape[2]).permute(1,0,2,3)
        if decoder_pass_taus:
            decoded_X = self.decoder_model(mean_sample,self.taus).squeeze(1)
        else:
